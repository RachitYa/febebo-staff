...
const ROLE_META = {
  'Cook':             { emoji:'≡ƒæ¿ΓÇì≡ƒì│', accent:'#a78bfa', accentBg:'#ede9fe', dept:'Kitchen & Mess',      grad:'#a78bfa' },
  'Cleaner':          { emoji:'≡ƒº╣',   accent:'#67e8f9', accentBg:'#cffafe', dept:'Housekeeping',        grad:'#67e8f9' },
  'Maintenance':      { emoji:'≡ƒ¢á∩╕Å',   accent:'#fda4af', accentBg:'#ffe4e6', dept:'Repairs & Technical', grad:'#fda4af' },
  'Purchase Manager': { emoji:'≡ƒ¢Æ',   accent:'#94a3b8', accentBg:'#f1f5f9', dept:'Store & Inventory',   grad:'#94a3b8' },
  'Security Guard':   { emoji:'≡ƒ¢í∩╕Å',   accent:'#6ee7b7', accentBg:'#d1fae5', dept:'Gate Security',       grad:'#6ee7b7' },
  'HR':               { emoji:'≡ƒæö',   accent:'#f472b6', accentBg:'#fce7f3', dept:'Human Resources & Hiring', grad:'#f472b6' },
  'Helper':           { emoji:'≡ƒÖï',   accent:'#fb923c', accentBg:'#fff7ed', dept:'General Helper',          grad:'#fb923c' },
  'Plumber':          { emoji:'≡ƒöº',   accent:'#60a5fa', accentBg:'#eff6ff', dept:'Plumbing & Water',        grad:'#60a5fa' },
  'Electrician':      { emoji:'ΓÜí',   accent:'#facc15', accentBg:'#fefce8', dept:'Electrical & Wiring',     grad:'#facc15' },
  'Carpenter':        { emoji:'≡ƒ¬Ü',   accent:'#a3a3a3', accentBg:'#fafafa', dept:'Carpentry & Fixtures',    grad:'#d4a574' },
  'Sales Manager':    { emoji:'≡ƒôê',   accent:'#34d399', accentBg:'#ecfdf5', dept:'Sales & Admissions',      grad:'#34d399' },
  'Manager':          { emoji:'≡ƒÅó',   accent:'#818cf8', accentBg:'#eef2ff', dept:'Operations Management',   grad:'#818cf8' },
  'Others':           { emoji:'ΓÜÖ∩╕Å',   accent:'#9ca3af', accentBg:'#f9fafb', dept:'General Staff',           grad:'#9ca3af' },
};
...
const INIT_CONTACTS = [
  {id:'c1', name:'Admin Office', role:'Admin', avatar:'≡ƒÅó', phone:'+91 99999 00000', isPinned:true, reminder:null, lastMsg:'Update daily logs by 6 PM today.', time:'09:00 AM'},
  {id:'c2', name:'Priya Sharma', role:'Student - Rm 102', avatar:'≡ƒæ⌐', phone:'+91 98888 77777', isPinned:false, reminder:'Check AC remote today 5:00 PM', lastMsg:'Can room 102 bathroom be cleaned at 11 AM?', time:'09:15 AM'},
  {id:'c3', name:'Dinesh (Maint.)', role:'Staff', avatar:'≡ƒ¢á∩╕Å', phone:'+91 97777 66666', isPinned:false, reminder:null, lastMsg:'Room 201 AC issue has been resolved.', time:'11:30 AM'},
];
...
const INIT_VISITORS = [
  {id:1,name:'Rajesh Malhotra',phone:'+91 98111 22233',purpose:'Parent Visit ΓÇô Rm 104',inTime:'10:15 AM',outTime:null,status:'Inside'},
  {id:2,name:'Zomato Delivery',phone:'+91 98222 33344',purpose:'Food Delivery ΓÇô Rm 202',inTime:'11:00 AM',outTime:'11:12 AM',status:'Exited'},
...
const INIT_TICKETS = [
  {id:1,room:'108',issue:'Water leakage ΓÇö bathroom sink tap',priority:'High',status:'Open',student:'Sneha Kapoor',date:'Today 09:30 AM'},
...
const INIT_CLEANING = [
  {id:1, room:'101', student:'Arjun Mehta', phone:'+91 9895921139', slot:'09:00 AM ΓÇô 11:00 AM', slotStatus:'completed', type:'Full Room Clean', done:true, note:'Please mop balcony too', date:'2026-07-24'},
  {id:2, room:'102', student:'Priya Sharma', phone:'+91 9861927774', slot:'11:00 AM ΓÇô 01:00 PM', slotStatus:'active', type:'Dusting & Mop', done:false, note:'Key with room partner', date:'2026-07-24'},
  {id:3, room:'105', student:'Ankit Kumar', phone:'+91 9848300363', slot:'11:00 AM ΓÇô 01:00 PM', slotStatus:'active', type:'Bathroom Sanitise', done:false, note:'Spilled juice near desk', date:'2026-07-24'},
  {id:4, room:'201', student:'Karan Singh', phone:'+91 9811122233', slot:'01:00 PM ΓÇô 03:00 PM', slotStatus:'upcoming', type:'Bedsheet & Towel Change', done:false, note:'Leave fresh towel on chair', date:'2026-07-24'},
  {id:5, room:'202', student:'Sneha Kapoor', phone:'+91 9822233344', slot:'03:00 PM ΓÇô 05:00 PM', slotStatus:'upcoming', type:'Full Room Clean', done:false, note:'Call before entering', date:'2026-07-24'},
  {id:6, room:'305', student:'Ravi Kumar', phone:'+91 9833344455', slot:'03:00 PM ΓÇô 05:00 PM', slotStatus:'upcoming', type:'Bathroom Sanitise', done:false, note:'', date:'2026-07-24'}
];
const INIT_DEMANDS = [
...
const INIT_CANDIDATES = [
  ...
  {id:3, name:'Vikram Rathi', position:'Security Guard', experience:'2 Yrs', phone:'+91 9899988877', status:'Hired Γ£à', date:'22 Jul', note:'Joined night shift'}
];

const INIT_ENQUIRIES = [
  {id:1, name:'Rohit Sharma', phone:'+91 9876543210', requirement:'Single Room AC', budget:'Γé╣10,000', status:'New ≡ƒö┤', source:'WhatsApp', text:'Looking for a single room with AC. Budget around Γé╣10,000.'},
  {id:2, name:'Priya Mehta', phone:'+91 9123456789', requirement:'Double Sharing', budget:'Γé╣7,500', status:'Contacted ≡ƒƒí', source:'NoBroker', text:'Do you have double sharing available near metro station?'},
  {id:3, name:'Arun Verma', phone:'+91 9012345678', requirement:'Triple Sharing', budget:'Γé╣6,000', status:'Closed ≡ƒƒó', source:'MagicBricks', text:'What is the security deposit for triple sharing?'},
  {id:4, name:'Kavya Singh', phone:'+91 9811122233', requirement:'Double Sharing AC', budget:'Γé╣8,500', status:'New ≡ƒö┤', source:'Direct Call', text:'Need immediate move-in from 1st August.'}
];

const INIT_TASKS = [
  {id:1, title:'Clean common area corridor ΓÇô Ground Floor', assignedBy:'Admin', priority:'High', status:'Pending', room:'Common', time:'09:00 AM'},
  ...
  {id:3, title:'Help guest luggage ΓÇô Room 204 checkout', assignedBy:'Admin', priority:'Normal', status:'Pending', room:'204', time:'12:00 PM'},
...
const INIT_PLUMBING = [
  ...
  {id:2, room:'302', issue:'Flush not working ΓÇô Tank valve stuck', student:'Gaurav Malhotra', priority:'Normal', status:'In Progress', date:'Today 10:00 AM', note:'Need new valve'},
...
const INIT_ELECTRICAL = [
  ...
  {id:2, room:'102', issue:'Fan running slow ΓÇô capacitor issue', student:'Priya Sharma', priority:'Normal', status:'Open', date:'Today 11:00 AM', note:''},
...
const INIT_CARPENTER = [
  ...
  {id:3, room:'301', issue:'Bed slat cracked ΓÇô squeaking', student:'Rahul Sharma', priority:'High', status:'In Progress', date:'Yesterday', note:'Replacement slat ordered'},
...
const INIT_ROOMS = [
  {id:1, number:'101', type:'Single AC', status:'Occupied', student:'Arjun Mehta', rent:'Γé╣12,000', due:'2026-08-01'},
  {id:2, number:'102', type:'Double Sharing', status:'Occupied', student:'Priya Sharma / Kavya', rent:'Γé╣8,500', due:'2026-08-01'},
  {id:3, number:'201', type:'Triple Sharing', status:'Occupied', student:'Karan / Rohit / Aman', rent:'Γé╣7,000', due:'2026-08-01'},
  {id:4, number:'203', type:'Single Non-AC', status:'Vacant', student:'ΓÇö', rent:'Γé╣9,000', due:'ΓÇö'},
  {id:5, number:'301', type:'Double Sharing AC', status:'Vacant', student:'ΓÇö', rent:'Γé╣10,500', due:'ΓÇö'},
  {id:6, number:'305', type:'Single AC', status:'Occupied', student:'Ravi Kumar', rent:'Γé╣12,000', due:'2026-08-01'},
];
...
// ΓöÇΓöÇΓöÇ Role-Based Personalized Item Lists ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇΓöÇ Small Reusable UI ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇΓöÇ Bottom Sheet Modal ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    <div style={{position:'fixed',inset:0,background:'rgba(15,23,42,.6)',zIndex:400,display:'flex',alignItems:'flex-end',backdropFilter:'blur(4px)'}} onClick={e=>{if(e.target===e.currentTarget)onClose();}}>
      <div style={{width:'100%',maxWidth:480,margin:'0 auto',background:'#fff',borderRadius:'24px 24px 0 0',padding:'0 0 36px',boxShadow: '0 4px 16px rgba(15,23,42,0.05)',animation:'sheetUp .25s ease'}}>
        ...
        <Row style={{padding:'12px 20px 16px',borderBottom: '1px solid #e2e8f0'}}>
          ...
          <button onClick={onClose} style={{background:C.bg,border: '1px solid #e2e8f0',borderRadius:10,width:32,height:32,display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer',fontSize:18,color:C.muted}}>Γ£ò</button>
        </Row>
...
export default function StaffApp(){
  ...
  const [bMsg,setBMsg]          = useState('Fresh hot meal is ready! Please head to the mess hall. ≡ƒì╜∩╕Å');
  ...
  const [rptHist,setRptHist]  = useState([{id:1,date:'22 Jul 2026',summary:'Completed mess clean-up & kitchen prep for dinner.',status:'Reviewed Γ£ô'}]);
  ...
  const [myReqs,setMyReqs]      = useState([
    ...
    {id:2,type:'Salary Advance',date:'20 Jul',status:'Approved',amt:'Γé╣3,000'},
  ]);
  ...
  const openReminder = () => {
    if (activeContact.reminder) {
      const parts = activeContact.reminder.split(' ΓÇö ');
      if (parts.length > 1) {
  ...
  const saveReminder = e => {
    e.preventDefault();
    const reminderStr = `${remReason ? `${remReason} ΓÇö ` : ''}${remDate} ${remTime}`;
  ...
  const submitRequest = e=>{
    ...
    setMyReqs(p=>[{id:Date.now(),type:reqType,date:new Date().toLocaleDateString('en-GB'),status:'Pending',amt:reqAmt?`Γé╣${reqAmt}`:'-'},...p]);
  ...
  const addVisitor = e=>{
    ...
    setVisitors(p=>[{id:Date.now(),name:vName,phone:vPhone||'ΓÇö',purpose:vPurp||'Visitor',inTime:new Date().toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'}),outTime:null,status:'Inside'},...p]);
  ...
  const addParcel = e=>{
    e.preventDefault();
    if(!pStu.trim()) return;
    setParcels(p=>[{id:Date.now(),student:pStu,room:pRoom||'ΓÇö',carrier:pCarr,tracking:pTrk||'TRK-'+Math.floor(1000+Math.random()*9000),date:'Today '+new Date().toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'}),status:'Pending'},...p]);
    setPStu(''); setPRoom(''); setPTrk(''); setShowParcel(false);
  };

  const eaten = students.filter(s=>s.statusB==='eaten').length; // Home view demo data

  // ΓöÇΓöÇΓöÇ Module tiles on Home ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  const MODULES = [
    ...
    {id:'salary',    label:'Salary',         sub:'Γé╣18,500 Jul',      icon:'payments',               grad:'#eef2ff'},
  ...
  // ΓöÇΓöÇΓöÇ Role quick stats ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  const roleStats = {
    'HR':               [{l:'Applicants',v:INIT_CANDIDATES.filter(c=>c.status!=='Hired Γ£à').length,icon:'group_add'},{l:'Enquiries',v:INIT_ENQUIRIES.length,icon:'contact_phone'},{l:'Staff Hired',v:INIT_CANDIDATES.filter(c=>c.status==='Hired Γ£à').length,icon:'badge'},{l:'Open Jobs',v:3,icon:'work_outline'}],
  ...
  return (
    <div style={{maxWidth:480,margin:'0 auto',minHeight:'100vh',background:C.bg,fontFamily:"'Hanken Grotesk',sans-serif",position:'relative',overflowX:'hidden'}}>
      ...
      {/* ΓöÇΓöÇ Sidebar Overlay ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
      ...
      {/* ΓöÇΓöÇ Sidebar ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
      ...
      {/* ΓöÇΓöÇ HEADER (always visible) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
      {view === 'home' ? (
        // Home Hero Header
        <div style={{background: 'linear-gradient(to bottom, #fffef2, #fffdf0)', padding:'0 16px 20px', color: '#1a1500', borderBottom: '1.5px solid #e8df9a'}}>
          ...
          <div style={{marginTop:14}}>
            <p style={{margin:0, fontSize:13, fontWeight:800, color:'#ca8a04', textTransform:'uppercase', letterSpacing:'0.04em'}}>{greet}, {firstName} ≡ƒæï</p>
      ...
      {view === 'home' && (
        <div style={{padding:'16px 14px', paddingBottom:96}}>
          ...
          <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:12}}>
            ...
            <span style={{fontSize:12, fontWeight:800, color:C.primaryDk, cursor:'pointer'}}>See all Γû╛</span>
          </div>
          ...
          <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:12}}>
            ...
            <span onClick={() => setView('work')} style={{fontSize:12, fontWeight:800, color:C.primaryDk, cursor:'pointer'}}>See all Γû╛</span>
          </div>
          ...
          <div style={{display:'flex', flexDirection:'column', gap:10}}>
            ...
            {staffRole === 'Cleaner' && cleaning.filter(c => !c.done).slice(0, 3).map(c => (
              <div key={c.id} onClick={() => setView('work')} style={{background:'#fff', border:`1px solid ${C.border}`, borderRadius:18, padding:14, display:'flex', alignItems:'center', justifyContent:'space-between', boxShadow:'0 4px 12px rgba(120, 104, 10, 0.03)', cursor:'pointer'}}>
                <div style={{display:'flex', alignItems:'center', gap:12}}>
                  <div style={{width:40, height:40, borderRadius:12, background:'#fefce8', display:'flex', alignItems:'center', justifyContent:'center', fontSize:20}}>≡ƒº╣</div>
                  <div>
            ...
            {staffRole === 'Cook' && (
              <div onClick={() => setView('work')} style={{background:'#fff', border:`1px solid ${C.border}`, borderRadius:18, padding:14, display:'flex', alignItems:'center', justifyContent:'space-between', boxShadow:'0 4px 12px rgba(120, 104, 10, 0.03)', cursor:'pointer'}}>
                <div style={{display:'flex', alignItems:'center', gap:12}}>
                  <div style={{width:40, height:40, borderRadius:12, background:'#fefce8', display:'flex', alignItems:'center', justifyContent:'center', fontSize:20}}>≡ƒæ¿ΓÇì≡ƒì│</div>
                  <div>
        ...
        <div style={{padding:'14px 14px 32px',display:'flex',flexDirection:'column',gap:14}}>
                           ...
                           <div style={{display:'flex', flexDirection:'column', gap:6, alignItems:'flex-end'}}>
                             {(selectedStat === 'pack' || selectedStat === 'extra') && (
                               <button onClick={()=>{
                                 ...
                                 const priceMatch = currentVal.match(/Γé╣(\d+)/);
                                 setPackPriceVal(priceMatch ? priceMatch[1] : '');
                                 setPackVal(currentVal.replace(/\s*\(Γé╣\d+\)/, '')); 
                             ...
                             {selectedStat === 'eaten' && timeFilter === 'Daily' && (
                                <Chip label="Eaten Γ£à" color="#166534" bg="#dcfce7"/>
                             )}
                           </div>
                         </div>
                       ))}
                ...
                {cleanerTimeFilter !== 'Daily' ? (
                  <div style={{display:'flex', flexDirection:'column', gap:14}}>
                    <div style={{background:C.primary, borderRadius:16, border: '1px solid #e2e8f0', padding:'16px', boxShadow: '0 4px 16px rgba(15,23,42,0.05)'}}>
                      <p style={{margin:0, fontSize:12, fontWeight:800, textTransform:'uppercase'}}>≡ƒôè {cleanerTimeFilter} Cleaning Report ({cleanerTypeFilter} Filter)</p>
                    ...
                    <div style={{background:'#fff', borderRadius:16, border: '1px solid #e2e8f0', padding:16, boxShadow: '0 4px 16px rgba(15,23,42,0.05)'}}>
                      <p style={{margin:'0 0 6px', fontSize:15, fontWeight:800, color:'#000'}}>Γ£à Cleaned Rooms Details ({cleanerTimeFilter})</p>
                      ...
                      <div style={{display:'flex', flexDirection:'column', gap:8}}>
                        {cleaning.filter(c => c.done && matchesType(c)).map(c => (
                          <div key={c.id} style={{background:'#f0fdf4', border: '1px solid #e2e8f0', borderRadius:12, padding:12, display:'flex', justifyContent:'space-between', alignItems:'center'}}>
                            ...
                            <Chip label="Cleaned Γ£ô" color="#166534" bg="#dcfce7"/>
                          </div>
                        ))}
                      </div>
                    </div>
                    ...
                    <div style={{background:'#fff', borderRadius:16, border: '1px solid #e2e8f0', padding:16, boxShadow: '0 4px 16px rgba(15,23,42,0.05)'}}>
                      <p style={{margin:'0 0 6px', fontSize:15, fontWeight:800, color:'#000'}}>ΓÅ│ Uncleaned / Pending Rooms ({cleanerTimeFilter})</p>
                      ...
                      <div style={{display:'flex', flexDirection:'column', gap:8}}>
                        {cleaning.filter(c => !c.done && matchesType(c)).map(c => (
                          <div key={c.id} style={{background:'#fff1f2', border: '1px solid #e2e8f0', borderRadius:12, padding:12, display:'flex', justifyContent:'space-between', alignItems:'center'}}>
                            ...
                            <Chip label="Pending ΓÅ│" color="#b91c1c" bg="#fee2e2"/>
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  <>
                    ...
                    <div style={{background:'#fef08a', borderRadius:14, border: '1px solid #e2e8f0', padding:'12px 16px', display:'flex', justifyContent:'space-between', alignItems:'center', boxShadow: '0 3px 12px rgba(15,23,42,0.05)'}}>
                      <div>
                        <span style={{fontSize:11, fontWeight:800, textTransform:'uppercase', color:'#000'}}>ΓÜí Current 2-Hour Cleaning Slot</span>
                        <h3 style={{margin:'2px 0 0', fontSize:18, fontWeight:900, color:'#000'}}>11:00 AM ΓÇô 01:00 PM</h3>
                      </div>
                    ...
                    <div style={{display:'flex', gap:6, overflowX:'auto', scrollbarWidth:'none'}}>
                      {[
                        ...
                        {id:'completed', label:'Cleaned Γ£ô'}
                      ].map(tab => (
                        <button 
                          key={tab.id} 
                    ...
                    <div style={{background:'#fff', borderRadius:18, border: '1px solid #e2e8f0', padding:16, boxShadow: '0 4px 16px rgba(15,23,42,0.05)'}}>
                      <p style={{margin:'0 0 12px', fontSize:15, fontWeight:800, color:'#000'}}>
                        ≡ƒº╣ Cleaning Schedule <span style={{fontSize:12, fontWeight:600, color:C.muted}}>({cleanerDate})</span>
                      </p>
                        ...
                        {filteredList.map(slot => (
                          <div key={slot.id} style={{background: slot.done ? '#f0fdf4' : slot.slotStatus==='active' ? '#fefce8' : '#fafafa', border: '1px solid #e2e8f0', borderRadius:14, padding:14, boxShadow: '0 2px 8px rgba(15,23,42,0.04)'}}>
                            ...
                            <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginTop:10, paddingTop:8, borderTop:'1px dashed #000'}}>
                              ...
                              {slot.done ? (
                                <Chip label="Cleaned Γ£à" color="#166534" bg="#dcfce7"/>
                              ) : (
                                <button 
          ...
          {staffRole === 'Maintenance' && (<>
            ...
            <div style={{background:'#fff',borderRadius:18,border: '1px solid #e2e8f0',padding:16}}>
              <p style={{margin:'0 0 12px',fontSize:15,fontWeight:800,color:C.text}}>≡ƒ¢á∩╕Å Repair Tickets</p>
              {tickets.map(t=>(
                <div key={t.id} style={{background:C.bg,border: '1px solid #e2e8f0',borderRadius:14,padding:14,marginBottom:10}}>
                  ...
                  {t.status !== 'Resolved' && (
                    <div style={{display:'flex',gap:8}}>
                      {t.status==='Open' && <button onClick={()=>setTickets(p=>p.map(x=>x.id===t.id?{...x,status:'In Progress'}:x))} style={{flex:1,padding:9,background:C.indigoBg,border:`1px solid ${C.indigo}`,borderRadius:10,color:C.indigo,fontSize:12,fontWeight:800,cursor:'pointer',fontFamily:'inherit'}}>Start Work ≡ƒöº</button>}
                      <button onClick={()=>setTickets(p=>p.map(x=>x.id===t.id?{...x,status:'Resolved'}:x))} style={{flex:1,padding:9,background: C.primary,border: '1px solid #e2e8f0',borderRadius:10,color:'#000',fontSize:12,fontWeight:800,cursor:'pointer',fontFamily:'inherit'}}>Mark Resolved Γ£à</button>
                    </div>
                  )}
                  {t.status==='Resolved' && <Chip label="Resolved Γ£à" color={C.success} bg={C.successBg}/>}
                </div>
              ))}
            </div>
          </>)}
          ...
          {staffRole === 'Purchase Manager' && (<>
            ...
            <div style={{display:'flex', background:'#fff', borderRadius:12, padding:4, border:`1px solid ${C.border}`, boxShadow:'0 2px 8px rgba(15,23,42,0.04)', marginBottom:14}}>
              <button onClick={()=>setPmTab('requisitions')} style={{flex:1, padding:'10px 0', borderRadius:10, border:'none', background: pmTab==='requisitions' ? '#fde047' : 'transparent', color: pmTab==='requisitions' ? '#78680a' : C.muted, fontSize:13, fontWeight:900, cursor:'pointer', fontFamily:'inherit'}}>
                ≡ƒ¢Æ Requisitions ({demands.filter(d=>d.status==='Pending').length})
              </button>
            ...
            {pmTab === 'requisitions' && (
              <div style={{background:'#fff',borderRadius:18,border: `1px solid ${C.border}`,padding:16, boxShadow:'0 4px 12px rgba(120, 104, 10, 0.04)'}}>
                <p style={{margin:'0 0 12px',fontSize:15,fontWeight:800,color:C.text}}>≡ƒôï Staff Item Requisitions</p>
                {demands.map(d=>(
                  <div key={d.id} style={{background:C.bg,border: `1px solid ${C.border}`,borderRadius:14,padding:12,marginBottom:8,display:'flex',justifyContent:'space-between',alignItems:'center',gap:10}}>
                    ...
                    {d.status==='Pending' ? (
                      <button onClick={()=>setDemands(p=>p.map(x=>x.id===d.id?{...x,status:'PO Sent'}:x))} style={{padding:'8px 12px',background:meta.accentBg,border:`1.5px solid ${meta.accent}`,borderRadius:10,color:meta.accent,fontSize:12,fontWeight:800,cursor:'pointer',fontFamily:'inherit',whiteSpace:'nowrap'}}>Create PO ≡ƒôª</button>
                    ) : (
                      <Chip label="PO Sent Γ£ô" color={C.success} bg={C.successBg}/>
                    )}
                  </div>
                ))}
              </div>
            ...
            {pmTab === 'vendors' && !selectedVendor && (
              <div style={{display:'flex', flexDirection:'column', gap:14}}>
                ...
                <div style={{background:'#ffffff', border:`1px solid ${C.border}`, borderRadius:18, padding:'16px 20px', boxShadow:'0 4px 12px rgba(120,104,10,0.03)'}}>
                  ...
                  <h2 style={{margin:'4px 0 0', fontSize:32, fontWeight:900, color:'#b91c1c'}}>Γé╣{vendors.reduce((s, v)=>s+v.balance, 0).toLocaleString()}</h2>
                </div>
                    ...
                    .map(v => (
                      ...
                      >
                        <div>
                          ...
                          <p style={{margin:0, fontSize:12, color:C.muted}}>Outstanding Balance: Γé╣{v.balance.toLocaleString()}</p>
                        </div>
                        <div style={{display:'flex', alignItems:'center', gap:4}}>
                          <span style={{fontSize:16, fontWeight:900, color:C.text}}>Γé╣{v.balance.toLocaleString()}</span>
            ...
            {pmTab === 'vendors' && selectedVendor && (
              <div style={{display:'flex', flexDirection:'column', gap:14}}>
                ...
                {selectedVendor.balance > 0 && (
                  <div onClick={() => {
                    ...
                    <div style={{display:'flex', alignItems:'center', gap:8}}>
                      ...
                      <span style={{fontSize:12.5, fontWeight:800, color:'#b91c1c'}}>Pending Amount ΓÇö Tap to Clear</span>
                    </div>
                    <span style={{fontSize:13, fontWeight:900, color:'#b91c1c'}}>Γé╣{selectedVendor.balance.toLocaleString()}</span>
                  </div>
                )}
                ...
                <div style={{background:'#ffffff', border:`1px solid ${C.border}`, borderRadius:18, padding:'16px 20px', boxShadow:'0 4px 12px rgba(120,104,10,0.03)'}}>
                  ...
                  <h2 style={{margin:'4px 0 0', fontSize:32, fontWeight:900, color:C.text}}>Γé╣{selectedVendor.balance.toLocaleString()}</h2>
                </div>

                {/* Transactions Ledger header */}
                <p style={{margin:'4px 0 0', fontSize:14, fontWeight:900, color:C.text}}>≡ƒôä Statement & Ledger History</p>
                      ...
                      <div key={l.id} style={{background:'#fff', border:`1px solid ${C.border}`, borderRadius:16, padding:14, boxShadow:'0 2px 8px rgba(120,104,10,0.03)'}}>
                        <div style={{display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:6}}>
                          ...
                          <div style={{textAlign:'right'}}>
                            <p style={{margin:0, fontSize:15, fontWeight:900, color: l.type==='Purchase'?'#b91c1c':'#166534'}}>{l.type==='Purchase'?'+':'-'} Γé╣{l.amount.toLocaleString()}</p>
          ...
          {staffRole === 'Security Guard' && (<>
            ...
            <div style={{background:'#fff',borderRadius:18,border: '1px solid #e2e8f0',padding:16}}>
              <Row style={{marginBottom:12}}><p style={{margin:0,fontSize:15,fontWeight:800,color:C.text}}>≡ƒ¢í∩╕Å Gate Entry Log</p><Chip label={`${visitors.filter(v=>v.status==='Inside').length} Inside`} color={C.success} bg={C.successBg}/></Row>
              {visitors.map(v=>(
          ...
          {(staffRole === 'Helper' || staffRole === 'Others') && (<>
            ...
            <div style={{background: isAvailable ? '#dcfce7' : '#fee2e2', borderRadius:16, border: `1px solid ${isAvailable ? '#86efac' : '#fca5a5'}`, padding:'14px 18px', display:'flex', justifyContent:'space-between', alignItems:'center', boxShadow:'0 4px 12px rgba(15,23,42,0.05)'}}>
              <div>
                <p style={{margin:0, fontSize:13, fontWeight:900, color: isAvailable ? '#166534' : '#991b1b'}}>
                  {isAvailable ? 'Γ£à You are Available' : '≡ƒö┤ Marked Unavailable'}
                </p>
            ...
            <div style={{background:'#fff', borderRadius:16, border:'1px solid #e2e8f0', padding:16, boxShadow:'0 4px 16px rgba(15,23,42,0.05)'}}>
              <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:14}}>
                <div>
                  <p style={{margin:0, fontSize:15, fontWeight:900, color:C.text}}>≡ƒôï My Tasks Today</p>
                  ...
                  <div key={t.id} style={{background: t.status==='Done' ? '#f0fdf4' : '#fafafa', border: `1px solid ${t.status==='Done' ? '#bbf7d0' : '#e2e8f0'}`, borderRadius:12, padding:14, display:'flex', justifyContent:'space-between', alignItems:'center', gap:12}}>
                    <div style={{flex:1}}>
                      ...
                      <div style={{display:'flex', gap:6, flexWrap:'wrap'}}>
                        ...
                        <Chip label={`ΓÅ░ ${t.time}`} color="#475569" bg="#f1f5f9"/>
                        <Chip label={`≡ƒôì ${t.room}`} color="#475569" bg="#f1f5f9"/>
                      </div>
                    ...
                    <button onClick={()=>setTasks(prev=>prev.map(x=>x.id===t.id?{...x,status:x.status==='Done'?'Pending':'Done'}:x))} style={{padding:'8px 12px', borderRadius:10, border:'none', background: t.status==='Done' ? '#dcfce7' : meta.accentBg, color: t.status==='Done' ? '#166534' : meta.accent, fontSize:11, fontWeight:900, cursor:'pointer', fontFamily:'inherit', whiteSpace:'nowrap', minWidth:60}}>
                      {t.status==='Done' ? 'Γå⌐ Undo' : 'Γ£ô Done'}
                    </button>
                  </div>
                ))}
              </div>
          ...
          {staffRole === 'Electrician' && (<>
            ...
            {electricalJobs.some(j=>j.priority==='High' && j.status!=='Resolved') && (
              <div style={{background:'#fffbfe', border:'1.5px solid #fde047', borderRadius:18, padding:'12px 16px', display:'flex', alignItems:'center', gap:10, boxShadow:'0 4px 12px rgba(253, 224, 71, 0.05)'}}>
                ...
                <p style={{margin:0, fontSize:12.5, fontWeight:800, color:'#854d0e'}}>High voltage / sparking issue reported ΓÇö use PPE before starting work!</p>
              </div>
            )}
                ...
                {enquiries.map(e => (
                  <div key={e.id} style={{background:'#fff', border:'1px solid #e2e8f0', borderRadius:14, padding:14, boxShadow:'0 4px 14px rgba(15,23,42,0.04)'}}>
                    ...
                    <div style={{display:'flex', gap:8}}>
                      ...
                      <button onClick={()=>setEnquiries(prev=>prev.map(x=>x.id===e.id?{...x,status:x.status.includes('Contacted')?'Closed ≡ƒƒó':'Contacted ≡ƒƒí'}:x))} style={{flex:2, padding:'9px 0', background:'#fefce8', border:'1px solid #fcd34d', borderRadius:10, color:'#78350f', fontSize:12, fontWeight:800, cursor:'pointer', fontFamily:'inherit'}}>
                        {e.status.includes('Contacted') ? 'Γ£ô Mark Converted' : '≡ƒô▓ Mark Contacted'}
                      </button>
                    </div>
                  </div>
                ))}
            ...
            {salesTab === 'rooms' && (<>
              <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, marginBottom:4}}>
                ...
                <div style={{background:'#fef3c7', borderRadius:14, border:'1px solid #fde68a', padding:14, textAlign:'center'}}>
                  ...
                  <p style={{fontSize:11, fontWeight:800, margin:'2px 0 0', color:'#92400e', textTransform:'uppercase'}}>Vacant ΓÇö Available</p>
                </div>
              </div>
          ...
          {staffRole === 'Manager' && (<>
            ...
            <div style={{background:'linear-gradient(135deg,#92400e,#d97706)', borderRadius:16, padding:'14px 16px', color:'#fff', boxShadow:'0 8px 20px rgba(146,64,14,0.18)'}}>
              <p style={{margin:0, fontSize:11, fontWeight:800, textTransform:'uppercase', letterSpacing:0.5, color:'#fef3c7'}}>≡ƒÅó Command Center</p>
              <h3 style={{margin:'4px 0 2px', fontSize:20, fontWeight:900}}>Operations Overview</h3>
              <p style={{margin:0, fontSize:12, color:'#fde68a', fontWeight:700}}>Febebo PG ΓÇö All departments</p>
            </div>
            ...
            <div style={{background:'#fff', borderRadius:16, border:'1px solid #e2e8f0', padding:16, boxShadow:'0 4px 16px rgba(15,23,42,0.05)'}}>
              <p style={{margin:'0 0 12px', fontSize:14, fontWeight:900, color:C.text}}>≡ƒöÄ Open Issues by Department</p>
              {[
                ...
                <div key={d.dept} style={{background:d.bg, border:`1px solid ${d.color}40`, borderRadius:12, padding:'10px 14px', marginBottom:8, display:'flex', justifyContent:'space-between', alignItems:'center'}}>
                  <div style={{display:'flex', alignItems:'center', gap:10}}>
                    ...
                    <div>
                      ...
                      {d.high > 0 && <p style={{margin:0, fontSize:11, color:'#b91c1c', fontWeight:700}}>{d.high} High Priority ΓÜá∩╕Å</p>}
                    </div>
                  </div>
            ...
            <div style={{background:'#fff', borderRadius:16, border:'1px solid #e2e8f0', padding:16, boxShadow:'0 4px 16px rgba(15,23,42,0.05)'}}>
              <p style={{margin:'0 0 12px', fontSize:14, fontWeight:900, color:C.text}}>≡ƒæÑ Staff On Duty Today</p>
              {['Cook (2)', 'Cleaner (3)', 'Maintenance (2)', 'Security (2)', 'Helper (3)'].map(s => (
                <div key={s} style={{display:'flex', justifyContent:'space-between', alignItems:'center', padding:'8px 0', borderBottom:'1px solid #f1f5f9'}}>
                  ...
                  <Chip label="On Duty Γ£à" color="#166534" bg="#dcfce7"/>
                </div>
              ))}
          ...
          {staffRole === 'HR' && (<>
            ...
            <div style={{display:'flex', background:'#fff', borderRadius:12, padding:4, border: '1px solid #e2e8f0', boxShadow: '0 3px 12px rgba(15,23,42,0.05)'}}>
              ...
              >
                ≡ƒæÑ Hiring & Candidates ({candidates.length})
              </button>
              <button 
              ...
              >
                ≡ƒôï Room Enquiries ({enquiries.length})
              </button>
            </div>
              ...
              <div style={{display:'flex', flexDirection:'column', gap:14}}>
                ...
                <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:10}}>
                  <div style={{background:'#fef08a', borderRadius:14, border: '1px solid #e2e8f0', padding:14, textAlign:'center', boxShadow: '0 3px 12px rgba(15,23,42,0.05)'}}>
                    <p style={{fontSize:24, fontWeight:900, margin:0, color:'#000'}}>{candidates.filter(c=>c.status!=='Hired Γ£à').length}</p>
                ...
                <div style={{background:'#fff', borderRadius:16, border: '1px solid #e2e8f0', padding:16, boxShadow: '0 4px 16px rgba(15,23,42,0.05)'}}>
                  <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:12}}>
                    <p style={{margin:0, fontSize:15, fontWeight:900, color:'#000'}}>≡ƒæö Candidate Recruitment Pipeline</p>
                    <button 
                      onClick={()=>{
                    ...
                    {candidates.map(c => (
                      <div key={c.id} style={{background:'#fafafa', border: '1px solid #e2e8f0', borderRadius:12, padding:14, boxShadow: '0 2px 8px rgba(15,23,42,0.04)'}}>
                        <div style={{display:'flex', justifyContent:'space-between', alignItems:'flex-start'}}>
                          ...
                          <span style={{fontSize:11, fontWeight:800, padding:'3px 8px', borderRadius:8, border: '1px solid #e8df9a', background: c.status==='Hired Γ£à'?'#bbf7d0':'#fef08a', color:'#000'}}>
                            {c.status}
                          </span>
                        </div>

                        {c.note && (
                          <p style={{margin:'8px 0 0', fontSize:11, fontWeight:700, color:'#444'}}>≡ƒÆ¼ {c.note}</p>
                        )}
                        ...
                        <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginTop:12, paddingTop:8, borderTop:'1px dashed #000'}}>
                          ...
                          {c.status !== 'Hired Γ£à' && (
                            <button 
                              onClick={() => setCandidates(prev => prev.map(x => x.id === c.id ? {...x, status:'Hired Γ£à'} : x))}
              ...
              <div style={{display:'flex', flexDirection:'column', gap:14}}>
                ...
                <div style={{background:'#fefce8', borderRadius:16, border: '1px solid #fcd34d', padding:'16px', color:'#0f172a', boxShadow: '0 4px 16px rgba(202,138,4,0.1)'}}>
                  <div style={{display:'flex', justifyContent:'space-between', alignItems:'center'}}>
                    <div>
                      <p style={{margin:0, fontSize:11, fontWeight:800, textTransform:'uppercase', color:'#92400e', letterSpacing:0.5}}>≡ƒÅá Room Enquiries & Leads</p>
                  ...
                  {enquiries.map(e => (
                    <div key={e.id} style={{background:'#fff', border: '1px solid #e2e8f0', borderRadius:16, padding:16, boxShadow: '0 4px 14px rgba(15,23,42,0.04)'}}>
                      ...
                      <div style={{display:'flex', gap:8, marginTop:8}}>
                        ...
                        <button 
                          onClick={()=>setEnquiries(prev=>prev.map(x=>x.id===e.id?{...x, status: x.status.includes('Contacted')?'Closed ≡ƒƒó':'Contacted ≡ƒƒí'}:x))}
                        ...
                        >
                          {e.status.includes('Contacted') ? 'Mark Closed ≡ƒƒó' : 'Mark Contacted ≡ƒƒí'}
                        </button>
                      </div>
                    </div>
                  ))}
        ...
        <div style={{padding:'14px 14px 32px',display:'flex',flexDirection:'column',gap:14}}>
          ...
          <div style={{background: C.primary, borderRadius:18, border: '1px solid #e2e8f0', padding:'20px 18px', color:'#000', boxShadow: '0 4px 16px rgba(15,23,42,0.05)'}}>
            ...
            <button onClick={punch} style={{padding:'10px 20px', background:'#fff', border: '1px solid #e2e8f0', borderRadius: 10, color:'#000', fontSize:13, fontWeight:800, cursor:'pointer', fontFamily:'inherit', boxShadow: '0 2px 8px rgba(15,23,42,0.04)'}}>
              {clocked?'ΓÅ╣ Punch Out Now':'Γû╢ Punch In'}
            </button>
          </div>
          ...
          {/* Punch Log */}
          <div style={{background:'#fff', borderRadius:18, border: '1px solid #e2e8f0', padding:16, boxShadow: '0 4px 16px rgba(15,23,42,0.05)'}}>
            <p style={{margin:'0 0 12px', fontSize:15, fontWeight:800, color:'#000'}}>≡ƒôï Punch Log</p>
            {punchLog.map(p=>(
              <div key={p.id} style={{background:'#fafafa', border: '1px solid #e2e8f0', borderRadius: 12, padding:12, marginBottom:10, display:'flex', justifyContent:'space-between', alignItems:'center', boxShadow: '0 2px 8px rgba(15,23,42,0.04)'}}>
                <div>
                  ...
                  <p style={{margin:'3px 0 0', fontSize:12, color:C.muted, fontWeight:600}}>In: <b style={{color:'#000'}}>{p.inT}</b> ┬╖ Out: <b style={{color:'#000'}}>{p.outT||'ΓÇö'}</b></p>
                </div>
                {p.hrs ? (
        ...
        return (
          <div style={{padding:'14px 14px 32px', display:'flex', flexDirection:'column', gap:14}}>
            ...
            <div style={{background: C.primary, borderRadius:18, border: '1px solid #e2e8f0', padding:'20px 18px', color:'#000', boxShadow: '0 4px 16px rgba(15,23,42,0.05)'}}>
              ...
              <h2 style={{margin:'6px 0 2px', fontSize:36, fontWeight:900, letterSpacing:-1}}>Γé╣{currentBalance.toLocaleString()}</h2>
            ...
            <div style={{background:'#fff', borderRadius:18, border: '1px solid #e2e8f0', padding:16, boxShadow: '0 4px 16px rgba(15,23,42,0.05)'}}>
              <p style={{margin:'0 0 4px', fontSize:15, fontWeight:800, color:'#000'}}>≡ƒöæ Assigned Assets & Handheld Items</p>
            ...
            <div style={{background:'#fff', borderRadius:18, border: '1px solid #e2e8f0', padding:16, boxShadow: '0 4px 16px rgba(15,23,42,0.05)'}}>
              <p style={{margin:'0 0 12px', fontSize:15, fontWeight:800, color:'#000'}}>≡ƒô£ Cash Transactions Log</p>
                    ...
                    <div>
                      ...
                      {log.senderUpi && log.receiverUpi && (
                        <div style={{marginTop:4, padding:'4px 8px', background:'#fef08a', borderRadius:6, border: '1px solid #e8df9a', fontSize:10, fontWeight:700, color:'#000'}}>
                          ≡ƒÆ╕ UPI: {log.senderUpi} Γ₧ö {log.receiverUpi}
                        </div>
                      )}
                    </div>
                    <span style={{fontSize:15, fontWeight:900, color: log.type==='credit' ? '#15803d' : '#b91c1c'}}>
                      {log.type==='credit' ? `+Γé╣${log.amount}` : `ΓêÆΓé╣${log.amount}`}
                    </span>
                  </div>
                ))}
              </div>
      ...
      {view === 'items' && (
        <div style={{padding:'14px 14px 32px',display:'flex',flexDirection:'column',gap:12}}>
          ...
          {myDemands.length>0 && (<>
            <p style={{margin:'8px 0 6px',fontSize:13,fontWeight:800,color:C.text}}>≡ƒôï My Demands</p>
            {myDemands.map(d=>(
      ...
      {/* ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
          CHAT ΓÇö WHATSAPP STYLE WITH REMINDERS
         ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ */}
      {view === 'chat' && (
          ...
          {!activeContact ? (
            ...
            <div style={{flex:1, display:'flex', flexDirection:'column', background:'#f8fafc', height:'100%'}}>
              ...
              <form onSubmit={sendMsg} style={{display:'flex',gap:8,padding:'12px 14px',background:'#fff',borderTop: '1px solid #e2e8f0',flexShrink:0}}>
                <input value={chatInput} onChange={e=>setChatInput(e.target.value)} placeholder="Type a messageΓÇª"
      ...
      {view === 'reports' && (
        <div style={{padding:'14px 14px 32px',display:'flex',flexDirection:'column',gap:14}}>
          <div style={{background:'#fff',borderRadius:18,border: '1px solid #e2e8f0',padding:16}}>
            <p style={{margin:'0 0 12px',fontSize:15,fontWeight:800,color:C.text}}>≡ƒô¥ Submit Today's Report</p>
            <form onSubmit={submitReport} style={{display:'flex',flexDirection:'column',gap:12}}>
              <InputField label="Work Summary *" textarea required rows={4} value={rptText} onChange={e=>setRptText(e.target.value)} placeholder="Tasks completed, issues found, items neededΓÇª"/>
              <button type="submit" style={{padding:13,background:meta.grad,color:'#000',border: '1px solid #e2e8f0',borderRadius:14,fontSize:14,fontWeight:800,cursor:'pointer',fontFamily:'inherit',boxShadow: '0 4px 16px rgba(15,23,42,0.05)'}}>Submit Report ≡ƒÜÇ</button>
            </form>
          </div>
          <div style={{background:'#fff',borderRadius:18,border: '1px solid #e2e8f0',padding:16}}>
            <p style={{margin:'0 0 12px',fontSize:15,fontWeight:800,color:C.text}}>≡ƒôï Past Reports</p>
            {rptHist.map(r=>(
      ...
      {view === 'requests' && (
        <div style={{padding:'14px 14px 32px',display:'flex',flexDirection:'column',gap:14}}>
          <div style={{background:'#fff',borderRadius:18,border: '1px solid #e2e8f0',padding:16}}>
            <p style={{margin:'0 0 12px',fontSize:15,fontWeight:800,color:C.text}}>≡ƒô¼ New Request</p>
            <form onSubmit={submitRequest} style={{display:'flex',flexDirection:'column',gap:12}}>
              ...
              {reqType==='Advance' && <InputField label="Amount Needed (Γé╣)" type="number" value={reqAmt} onChange={e=>setReqAmt(e.target.value)} placeholder="e.g. 2000"/>}
              <InputField label="Reason / Details *" required value={reqReason} onChange={e=>setReqReason(e.target.value)} placeholder="Briefly explain your requestΓÇª"/>
              <button type="submit" style={{padding:13,background:meta.grad,color:'#000',border: '1px solid #e2e8f0',borderRadius:14,fontSize:14,fontWeight:800,cursor:'pointer',fontFamily:'inherit',boxShadow: '0 4px 16px rgba(15,23,42,0.05)'}}>Submit Request ≡ƒÜÇ</button>
            </form>
          </div>
          <div style={{background:'#fff',borderRadius:18,border: '1px solid #e2e8f0',padding:16}}>
            <p style={{margin:'0 0 12px',fontSize:15,fontWeight:800,color:C.text}}>≡ƒôï My Requests</p>
            {myReqs.map(r=>(
      ...
      {/* ΓöÇΓöÇ MODALS ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
      ...
      <Sheet show={showDemandList} onClose={()=>setShowDemandList(false)} title="Staff Requisitions Queue" sub="Item demands raised by staff members">
        <div style={{display:'flex', flexDirection:'column', gap:14}}>
          <button onClick={()=>{ setShowDemandForm(true); setShowDemandList(false); }} style={{width:'100%', padding:12, background:C.primaryBg, border:`1.5px solid ${C.border}`, borderRadius:12, color:C.primaryDk, fontSize:13, fontWeight:800, cursor:'pointer', fontFamily:'inherit'}}>
            Γ₧ò Create New Demand / Requisition
          </button>
                ...
                <div key={d.id} style={{background:'#fff', border:`1px solid ${C.border}`, borderRadius:14, padding:14, boxShadow:'0 2px 8px rgba(120,104,10,0.03)'}}>
                  <div style={{display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:8}}>
                    <div>
                      ...
                      <p style={{margin:'2px 0 0', fontSize:10.5, color:C.muted}}>≡ƒôà {d.date} ┬╖ Vendor: {d.vendor}</p>
                    </div>
      ...
      <Sheet show={showAddPurchaseModal} onClose={()=>setShowAddPurchaseModal(false)} title="Add Items" sub={selectedVendor ? `${selectedVendor.category} ┬╖ ${selectedVendor.shop}` : ''}>
        <div style={{display:'flex', flexDirection:'column', gap:12}}>
            ...
            {purchaseItemsState.map((item, idx) => (
              <div key={item.name} style={{display:'flex', alignItems:'center', justifyContent:'space-between', padding:'8px 10px', background:'#fff', border:`1px solid ${C.border}`, borderRadius:12}}>
                ...
                <div style={{flex:0.8, textAlign:'right', fontSize:13, fontWeight:900, color:C.text}}>
                  {item.checked ? `Γé╣${item.qty * item.rate}` : '-'}
                </div>
              </div>
            ))}
          </div>
          ...
          <div style={{background:'#ecfeff', border:'1px solid #cffafe', borderRadius:12, padding:'10px 14px', display:'flex', justifyContent:'space-between', alignItems:'center'}}>
            ...
            <span style={{fontSize:14, fontWeight:900, color:'#0891b2'}}>Γé╣{purchaseItemsState.filter(i=>i.checked).reduce((s, i) => s + (i.qty * i.rate), 0)}</span>
          </div>
        ...
        <div style={{display:'flex', flexDirection:'column', gap:12}}>
          ...
          <div style={{background:'#fafafa', border:`1px solid ${C.border}`, borderRadius:14, padding:14, display:'flex', flexDirection:'column', gap:6}}>
            <div style={{display:'flex', justifyContent:'space-between', fontSize:13}}>
              ...
              <span style={{fontWeight:800, color:C.text}}>Γé╣{selectedVendor?.balance.toLocaleString()}</span>
            </div>
            <div style={{display:'flex', justifyContent:'space-between', fontSize:13}}>
              <span style={{color:C.muted}}>Remaining after payment</span>
              <span style={{fontWeight:900, color:'#b91c1c'}}>Γé╣{(Number(selectedVendor?.balance || 0) - Number(payAmount || 0)).toLocaleString()}</span>
            </div>
          </div>

          {/* Amount field */}
          <InputField 
            label="Amount Paying Now (Γé╣)" 
            type="number" 
            required 
            value={payAmount} 
          ...
          <button 
            onClick={() => {
              ...
              alert(`Payment of Γé╣${amt.toLocaleString()} recorded successfully!`);
            }} 
            style={{width:'100%', padding:14, background:'#0891b2', border:'none', borderRadius:12, color:'#fff', fontSize:14, fontWeight:900, cursor:'pointer', fontFamily:'inherit'}}
          >
            Confirm Payment
      ...
      <Sheet show={showDemandForm} onClose={()=>setShowDemandForm(false)} title="Demand Item / Supplies" sub="Submit requisition to admin">
        <form onSubmit={submitDemand} style={{display:'flex',flexDirection:'column',gap:12}}>
          ...
          <button type="submit" style={{padding:14,background:C.primary,color:C.text,border:`1.5px solid ${C.border}`,borderRadius:14,fontSize:14,fontWeight:800,cursor:'pointer',fontFamily:'inherit',boxShadow:'0 4px 16px rgba(15,23,42,0.05)'}}>Submit Demand ≡ƒÜÇ</button>
        </form>
      </Sheet>

      <Sheet show={showBcast} onClose={()=>setShowBcast(false)} title="Broadcast ΓÇö Food is Ready!" sub="Send meal notification">
        <form onSubmit={e=>{
          e.preventDefault();
          setShowBcast(false);
          const targetName = bTarget === 'All' ? 'All Students' : students.find(s=>s.id===Number(bStudentId))?.name;
          alert(`≡ƒôó ${bMeal} broadcast sent to ${targetName}!`);
          ...
          <button type="submit" style={{padding:14,background: C.primary,color:'#000',border: '1px solid #e2e8f0',borderRadius:14,fontSize:14,fontWeight:800,cursor:'pointer',fontFamily:'inherit',boxShadow: '0 4px 16px rgba(15,23,42,0.05)'}}>Send Notification ≡ƒôó</button>
        </form>
      </Sheet>
      ...
      <Sheet show={showPackEdit} onClose={()=>setShowPackEdit(false)} title={selectedStat === 'extra' ? "Extra Plate Details" : "Pack Details"} sub={`For ${students.find(s=>s.id===packStudentId)?.name || 'Student'}`}>
        <form onSubmit={e=>{
          e.preventDefault();
          const detailsKey = mealTab==='Breakfast'?'detailsB':mealTab==='Lunch'?'detailsL':mealTab==='Snacks'?'detailsS':'detailsD';
          const formattedVal = (selectedStat === 'extra' && packPriceVal) ? `${packVal} (Γé╣${packPriceVal})` : packVal;
          ...
          {selectedStat === 'extra' && (
            <InputField label="Extra Plate Charge / Money (Γé╣)" type="number" value={packPriceVal} onChange={e=>setPackPriceVal(e.target.value)} placeholder="e.g. 60"/>
          )}
      ...
      <Sheet show={showVisitor} onClose={()=>setShowVisitor(false)} title="Gate Visitor Entry" sub="Log a visitor at the gate">
        <form onSubmit={addVisitor} style={{display:'flex',flexDirection:'column',gap:12}}>
          ...
          <InputField label="Purpose & Room #" value={vPurp} onChange={e=>setVPurp(e.target.value)} placeholder="e.g. Parent visit ΓÇô Rm 104"/>
          <button type="submit" style={{padding:14,background:meta.grad,color:'#000',border: '1px solid #e2e8f0',borderRadius:14,fontSize:14,fontWeight:800,cursor:'pointer',fontFamily:'inherit',boxShadow: '0 4px 16px rgba(15,23,42,0.05)'}}>Check-In Visitor Γ£à</button>
        </form>
      </Sheet>
      ...
      <Sheet show={showParcel} onClose={()=>setShowParcel(false)} title="Log Courier Parcel" sub="Record parcel received at gate">
        <form onSubmit={addParcel} style={{display:'flex',flexDirection:'column',gap:12}}>
          ...
          <button type="submit" style={{padding:14,background: C.primary,color:'#000',border: '1px solid #e2e8f0',borderRadius:14,fontSize:14,fontWeight:800,cursor:'pointer',fontFamily:'inherit',boxShadow: '0 4px 16px rgba(15,23,42,0.05)'}}>Log Parcel ≡ƒôª</button>
        </form>
      </Sheet>
      ...
      <Sheet show={showExpenseModal} onClose={()=>setShowExpenseModal(false)} title="Log Expense / Item Purchase" sub="Deduct from available Petty Cash fund">
        <form onSubmit={e=>{
          ...
          <InputField label="Amount Spent (Γé╣) *" type="number" required value={expAmt} onChange={e=>setExpAmt(e.target.value)} placeholder="e.g. 500"/>
          ...
          {expMode === 'UPI / GPay' && (
            <div style={{background:'#fafafa', border: '1px solid #e2e8f0', borderRadius:10, padding:12, display:'flex', flexDirection:'column', gap:10}}>
              <p style={{margin:0, fontSize:12, fontWeight:800, color:'#000'}}>≡ƒô▓ UPI Transaction IDs</p>
      ...
      <Sheet show={showAddFundModal} onClose={()=>setShowAddFundModal(false)} title="Receive Advance Funds" sub="Add cash/UPI received from Admin or Students">
        <form onSubmit={e=>{
          ...
          <InputField label="Amount Received (Γé╣) *" type="number" required value={fundAmt} onChange={e=>setFundAmt(e.target.value)} placeholder="e.g. 2000"/>
